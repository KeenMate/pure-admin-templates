...
├── App.svelte            # Main layout (navbar, sidebar, footer, profile panel, router)
├── routes/
│   ├── index.ts          # Route definitions (defineRoutes)
│   ├── Dashboard.svelte  # Dashboard (/)
│   ├── Users.svelte      # Users page (/users)
│   ├── Settings.svelte   # Settings page (/settings)
│   └── GettingStarted.svelte  # This page (/getting-started)
index.html                # HTML shell (page loader, FOUC prevention, theme CSS)
public/
...
<!-- Routing -->
<Card titleText="Routing (@keenmate/svelte-spa-router)">
	<Paragraph class="mb-4">
		This template uses <strong>@keenmate/svelte-spa-router</strong> with hash-based URLs (<code>#/path</code>). Routes are defined in <code>src/routes/index.ts</code> using <code>defineRoutes</code> for type safety.
	</Paragraph>

	<Heading level={4}>Adding a New Page</Heading>
	<BasicList>
		<li>Create a new Svelte component in <code>src/routes/</code> (e.g., <code>Orders.svelte</code>)</li>
		<li>Register it in <code>src/routes/index.ts</code></li>
		<li>Add a <code>SidebarItem</code> in <code>App.svelte</code> with <code>href="#/orders"</code></li>
		<li>Optionally add a nav link with <code>use:link</code></li>
	</BasicList>

	<Heading level={4} class="mt-4">Example: Adding an Orders Page</Heading>
	<Paragraph>1. Create the component:</Paragraph>
	<CodeBlock>{`<!-- src/routes/Orders.svelte -->
<script lang="ts">
...
<Card titleText="Orders">
  <!-- Your content here -->
	...
	<Paragraph class="mt-4">2. Register the route in <code>src/routes/index.ts</code>:</Paragraph>
	<CodeBlock>{`import Orders from './Orders.svelte';
...
export const { routes, nav, paths } = defineRoutes({
  ...
  orders: {
	...
	<Paragraph class="mt-4">3. Add the sidebar item in <code>App.svelte</code>:</Paragraph>
...
<!-- Routing Features -->
<Card titleText="Router Features">
	<Grid>
		<Column size="100" lg="1-2">
			<Heading level={4}>Navigation</Heading>
			<CodeBlock>{`// Programmatic navigation
import { push, replace, pop } from '@keenmate/svelte-spa-router';

await push('/users');           // Navigate
await replace('/login');        // Replace (no history)
await pop();                    // Back

// Type-safe with defineRoutes
import { nav } from './routes';
await nav.users.push();
await nav.userDetail.push({ userId: 123 });`}</CodeBlock>
		</Column>
		<Column size="100" lg="1-2">
			<Heading level={4}>Link Action</Heading>
			<CodeBlock>{`<script>
  import { link } from '@keenmate/svelte-spa-router';
  import active from '@keenmate/svelte-spa-router/active';
</script>

<!-- Hash-based link with active highlighting -->
<a href="/users" use:link use:active>Users</a>

<!-- Named route -->
<a use:link={{route: 'userDetail', params: {userId: 123}}}>
  View User
</a>`}</CodeBlock>
		</Column>
	</Grid>

	<Heading level={4} class="mt-4">Route Parameters & Query</Heading>
	<CodeBlock>{`import { routeParams, query, location } from '@keenmate/svelte-spa-router';

// Reactive values (use with $derived)
const params = $derived(routeParams());    // { id: '123' }
const q = $derived(query());              // { tab: 'profile' }
const path = $derived(location());        // '/users/123'

// Or receive as props in route components
let { routeParams = {} } = $props();`}</CodeBlock>
</Card>

<!-- Layout -->
<Card titleText="Layout Components">
	<Paragraph class="mb-4">The layout in <code>App.svelte</code> is built from these components:</Paragraph>
	...
	<Table isCompact>
		...
		<tbody>
			...
			<tr><td><code>Main</code></td><td>Main content area (contains <code>&lt;Router&gt;</code>)</td></tr>
...
<Card titleText="Theming">
	<Paragraph class="mb-4">Themes are managed via the <code>pureadmin</code> CLI and loaded as static CSS files from <code>public/themes/</code>.</Paragraph>
...
npx pureadmin update            # Update all downloaded themes`}</CodeBlock>
	...
	<Paragraph>Change the theme CSS <code>&lt;link&gt;</code> in <code>index.html</code>:</Paragraph>
	<CodeBlock>{`<!-- In index.html <head> -->
...
<Card titleText="Resources">
	<BasicList>
		...
		<li><strong>Router:</strong> <code>@keenmate/svelte-spa-router</code> — hash/history SPA router for Svelte 5</li>
